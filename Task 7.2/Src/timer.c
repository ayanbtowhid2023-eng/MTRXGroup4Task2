...
{
    TIM3->SR &= ~TIM_SR_UIF;   // SR is the status register, when an interrupt fires it clears the UIF (Update Interrupt Flag) to 0 and allows for next interrupt to fire, otherwise the ISR enters an infinite loop
...
void timer_init(uint32_t period_ms_arg, TimerCallback callback) // Two inputs: period_ms_arg (desired period in ms), callback (function pointer for callback)
{
    ...
    __disable_irq(); // Disables the interrupts globally, such that no interrupts may fire while we are configuring the timer

    TIM3->CR1  &= ~TIM_CR1_CEN; // Clears the Counter Enable bit (CEN). Stops the timer so it doesn't behave unexpectedly during configuration.
    TIM3->PSC   = 7999; // Sets the pre-scaler dividing the 8MHz clock for TIM3 by 8000, such that it is 1 kHz. This means that now each 'tick' is 1ms. It is +1 as the pre-scaler automatically adds 1
    ...
    TIM3->EGR  |= TIM_EGR_UG; // Writing the UG bit of the EGR (Event Generation Register), loads shadow registers to allow for immediate effect
    TIM3->SR   &= ~TIM_SR_UIF; // Writing to the EGR may have set the status register (interrupt flag), this prevents any undesired behaviour by clearing it
    TIM3->DIER |= TIM_DIER_UIE; // Enables the DMA/Interrupt Enable Register. Tells TIM3 to generate an interrupt request when it overflows (i.e. finishes one round of counting)
    ...
    TIM3->CR1  |= TIM_CR1_CEN; // Sets CEN (Counter Enable) --> Counter begins counting up from 0 to ARR, then fires the interrupt and then resets

    __enable_irq(); // Enables interrupts to occur since configuration has completed
}
