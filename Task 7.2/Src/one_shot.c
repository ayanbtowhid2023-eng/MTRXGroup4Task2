...
{
    TIM4->SR  &= ~(1U << 0); // Clears the update interrupt flag to prevent the ISR from firing again immediately
    TIM4->CR1 &= ~(1U << 0); // Stops TIM4 by clearing the CEN (Counter Enable) - This is what ensures that the interrupt is only fired once
...
void one_shot_trigger(uint32_t delay_ms, OneShotCallback callback) // Two inputs: Delay in ms and the function pointer for the callback function
{
    ...
    stored_callback = callback; // Saves callback funciton pointer so ISR can call it after the delay

    RCC->APB1ENR |= RCC_APB1ENR_TIM4EN; // Enables TIM4's clock

    TIM4->CR1  &= ~(1U << 0); // Stops TIM4 while configuration is occurring (just in case it was running)
    TIM4->PSC   = 7999; // Pre-scaler to scale it down from 8MHz by 8000 to 1KHz, thus one tick per second
    ...
    TIM4->EGR  |= (1U << 0); // Sets the shadow registers
    TIM4->SR   &= ~(1U << 0); // Clears status register flag
    TIM4->DIER |= (1U << 0); // Enables update interrupt so when TIM4 overflows it fires an interrupt
    ...
    TIM4->CR1  |= (1U << 0); // Sets CEN (Counter Enable) to begin the counter
}
