...
static int validate(GPIO_t *port, uint8_t pin)
{
    if (port == 0) return -1;
    if (pin > 15)  return -1;
    return 0;
...
static int enable_clock(GPIO_t *port)
{
...
int gpio_init(GPIO_t *port, uint8_t pin, GPIO_Direction direction)
{
...
int gpio_init_af(GPIO_t *port, uint8_t pin, uint8_t af)
{
    if (validate(port, pin) != 0) return -1;
    if (af > 15)                  return -1;
    ...
    uint8_t reg    = pin >> 3;           /* 0 for pins 0-7, 1 for pins 8-15 */
    uint8_t offset = (pin & 0x7) * 4;   /* 4 bits per pin */
...
int gpio_write(GPIO_t *port, uint8_t pin, GPIO_PinState_t state)
{
    if (validate(port, pin) != 0) return -1;
...
int gpio_read(GPIO_t *port, uint8_t pin, GPIO_PinState_t *state)
{
    if (validate(port, pin) != 0) return -1;
    if (state == 0)                return -1;
