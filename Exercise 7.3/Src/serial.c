 ...
 *
 * Based directly on working reference implementation.
 * stm32f303xc.h is included for USART_TypeDef, GPIO_TypeDef, CMSIS
 * intrinsics (__disable_irq etc) and peripheral bit definitions.
 * registers.h is included alongside for the peripheral base address
 * macros (USART1, GPIOC, RCC etc) and clock enable bit definitions.
 *
 * ISR: USART1_EXTI25_IRQHandler -- confirmed correct name on this board.
 *
 * Task e note: receiveMsg() reads from the circular buffer filled by the
 * ISR. The checksum it verifies is XOR of the full packet bytes
 * (START, SIZE, TYPE, BODY..., STOP) -- this must match sendMsg().
 */
 ...
 * Values confirmed working on STM32F3 Discovery.
 * --------------------------------------------------------------------- */
 ...
 * USART1 ISR
 *
 * Name: USART1_EXTI25_IRQHandler
 * This is the name the vector table in startup_stm32f303vctx.s expects.
 *
 * Handles:
 *   RX: stores byte in circular buffer (Task e) or
 *       feeds packet state machine (Task f)
 *   TX: interrupt-driven transmit (Task f)
 *   TC: transmission complete (Task f)
 * --------------------------------------------------------------------- */
void USART1_EXTI25_IRQHandler(void)
{
    /* Clear overrun -- if not cleared UART stops receiving */
...
/* -----------------------------------------------------------------------
 * Task d) -- polling RX (reads hardware RXNE directly, no buffer)
 * --------------------------------------------------------------------- */
                       ...
                       void (*callback)(uint8_t *data, uint8_t size, uint8_t type))
{
    ...
    do { byte = SerialReceiveByte(serial_port); } while (byte != START_BYTE);
 ...
 *
 * Blocks indefinitely waiting for each byte from the circular buffer.
 * The ISR fills the buffer as bytes arrive -- no timeout needed since
 * we know a packet was sent and will arrive via loopback.
 * --------------------------------------------------------------------- */
                ...
                void (*callback)(uint8_t *data, uint8_t size, uint8_t type))
{
    ...
    /* Wait for START byte -- block until ISR puts it in buffer */
    do { } while (!buffer_read(&byte) || byte != START_BYTE);
    ...
    /* Verify -- XOR of all packet bytes START through STOP */
                            ...
                            void (*callback)(uint8_t *data, uint8_t size, uint8_t type))
{
    ...
    __disable_irq();
    if (!s_rx_message_ready) { __enable_irq(); return; }
